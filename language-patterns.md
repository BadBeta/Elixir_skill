...
Design functions with data as first argument:
...
Don't use pipelines for single operations: `result = process(input)` not `input |> process()`.

### Data-First Functions

```elixir
defmodule StringHelpers do
  # All functions take the data to transform as first argument
...
```elixir
# Extract meaningful intermediate steps
...
defp calculate_totals(orders) do
